...
apply: etrans (_ : \prod_(j in 'F_p | j < f j) (j * f j) = _ %[mod p]).
        ...
        \begin{itemize}
        \item O primeiro \textit{goal} a ser provado é:
...
Lemma big1 {R : Type} {idx : R} {op : Monoid.law idx} I r (P : pred I) F :
        ...
        \item O segundo \textit{goal} a ser provado é:
        
                \begin{lstlisting}[language=coq,frame=single,tabsize=1]
\prod_(j in 'F_p | j < f j) (j * f j)  = a' ^ p.-1./2  %[mod p]
                \end{lstlisting}

                para qual é inicialmente utilizada a aplicação do lema \lstinline[language=coq]|etrans| novamente:
...
apply: etrans (_ : \prod_(j in 'F_p | j < f j) a' = _ %[mod p]).
        ...
        (*... prova do goal 2...*)
                ...
                e assim, como feito em item anterior, têm-se agora dois \textit{goals} a serem resolvidos, dos quais o primeiro é:
...
\prod_(j in 'F_p | j < f j) (j * f j)  
        = \prod_(j in 'F_p | j < f j) a'  %[mod p]
                ...
                e são inicialmente, para este \textit{goal}, aplicadas as seguintes táticas:
