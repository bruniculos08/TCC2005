...
Lemma prod_nat_const n : \prod_(i in A) n = n ^ #|A|.
                                ...
                                com isto o lado direito do goal se torna:

                                \begin{lstlisting}[language=coq,frame=single,tabsize=1]
a' ^ #|(fun i : fintype_ordinal__canonical__fintype_Finite (Zp_trunc (pdiv p)).+2 => eqn (i.+1 - f i) 0)|
                                \end{lstlisting}
...
eqn (i.+1 - f i) 0
                                \end{lstlisting}
                                é a operação \lstinline[language=coq]|<|.


                                \item[\textbf{(4)}] Tranformação do \textit{goal} em uma igualdade entre os expoentes, dado que em cada lado da equação do \textit{goal} haviam exponenciações de mesma base.
