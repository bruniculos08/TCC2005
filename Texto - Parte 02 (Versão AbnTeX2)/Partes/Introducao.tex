...
Um conteúdo que carece de formalizações e provas, e possu relação com o conteúdo a ser apresentado neste trabalho, é o algoritmo de Tonelli-Shanks, também conhecido como Algoritmo \hyperref[algo:ressol]{\textsc{Ressol}} \cite{Huynh1581080}, acrônimo este que significa \textit{Residue Solver} de acordo com \cite{Niven1915}. Esse método resolve congruências quadráticas, isto é, equações da seguinte forma:
\begin{equation}
...
Essas considerações (sobre utilidades) valem portanto para qualquer algoritmo que resolve congruências quadráticas, e além disso, no que se trata sobre a relação do Algoritmo \hyperref[algo:ressol]{\textsc{Ressol}} (e possivelmente de outros métodos que possuem o mesmo objetivo), o \textit{símbolo de Legendre} se relaciona com tal por ser uma função que indica a existência ou não de uma solução para um instância da Equação \ref{eq:congruenciaquadratica}.
...
\begin{enumerate}
    ...
    \item A formalização deste conteúdo possibilita futuros trabalhos sobre conteúdos relevantes ainda não implementados na biblioteca como o já mencionado Algoritmo \hyperref[algo:ressol]{\textsc{Ressol}} (sobre o qual não há implementação em \textit{Coq}) e a \textit{Lei de Reciprocidade Quadrática}. 
