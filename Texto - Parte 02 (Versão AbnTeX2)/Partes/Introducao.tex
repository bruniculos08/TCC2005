...
Um dos conceitos mais presentes em Teoria dos Números é o de congruência que define uma relação entre dois números que divididos por um mesmo módulo possuem o mesmo resto (onde o resto é sempre um inteiro positivo menor que os dividendos). Essa relação é representada como: $a \equiv b \pmod{m}$, significando que $a$ é congruente a $b$ no módulo $m$, e é equivalente a dizer que $m \mid a - b$, isto é, $m$ divide $a - b$\footnote{A notação para dizer que um número $m$ não divide outro número $n$ será usada neste texto, e é dada por $m \nmid n$.}.
